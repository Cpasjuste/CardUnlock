...
int main(int argc, char *argv[]) {
    ...
    printf("EnsoTaiConfigReset by cpasjuste\n");
    printf("================================\n");
    printf("\n");
    ...
    printf("Removing config.txt\n");
    fd = sceIoOpen("xmc0:tai/config.txt", SCE_O_RDONLY, 0);
    if (fd >= 0) {
        sceIoClose(fd);

        ret = sceIoRemove("xmc0:tai/config.txt");
        if (ret != 0) {
            psvDebugScreenSetFgColor(COLOR_RED);
            psvDebugScreenPrintf("Unable to remove config.txt, err=0x%08X\n", ret);
            psvDebugScreenSetFgColor(COLOR_WHITE);
        }
    } else {
        psvDebugScreenPrintf("Unable to open config.txt, err=0x%08X\n", fd);
    }
    ...
    if (fd >= 0) {
        ...
        psvDebugScreenPrintf("Unable to open id.dat, err=0x%08X\n", fd);
    }
